...
input <- read.csv('DB_SEPI_COMPLETE_CLEANED.csv')
...
ggplot(df_calc, aes(x = Treatment, y = normalize)) +
  ...
  theme_classic() +
  labs(title = "All Test Types By Time Series - Bacteria", y = "Viable/Total Bacteria") +
  scale_y_continuous(breaks = c(0, 0.25, 0.5, 0.75, 1), limits = c(-0.01, 1.1)) +
  scale_fill_manual(values = c(pal[2], pal[3])) +
  theme(legend.title = element_blank())
...
ggplot(df_calc, aes(x = Treatment, y = normalize)) +
  ...
  geom_signif(comparisons = list(c("Control", "Experimental")),
              ...
              test = t.test, y_position = 1) +
  theme_classic() +
  labs(title = "All Samples By Treatment - Bacteria", y = "Viable/Total Bacteria") +
  scale_y_continuous(breaks = c(0, 0.25, 0.5, 0.75, 1), limits = c(-0.01, 1.1)) +
  scale_fill_manual(values = c(pal[2], pal[3])) +
  theme(legend.title = element_blank())
...
ggplot(input_cleaned, aes(x = Treatment, y = PMAcounts)) +
  ...
  geom_signif(comparisons = list(c("Control", "Experimental")),
              map_signif_level = FALSE, tip_length = 0.01,
              test = t.test, y_position = 100) +
  facet_wrap(~Time.Series) +
  theme_classic() +
  labs(title = "All Test Types By Time Series - Viable Bacteria Only", y = "Viable/Total Bacteria") +
  scale_y_continuous(, limits = c(-.01 , 200)) +
  scale_fill_manual(values = c(pal[2], pal[3])) +
  theme(legend.title = element_blank())
