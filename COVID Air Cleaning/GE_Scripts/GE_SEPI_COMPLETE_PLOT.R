...
hiflow <- df_calc[grepl('Air Sampler A - 200 l/m|Air Sampler B - 200 l/m', df_calc$Sample.Type), ]
...
loflow <- df_calc[grepl('Air Sampler C - 10 l/m|Air Sampler D - 8 l/m|Air Sampler E - 1.5 l/m', df_calc$Sample.Type), ]
...
ggplot(input_cleaned, aes(x = Treatment, y = log10(PMAcounts),)) +
  ...
  theme_classic() +
  labs(title = "All Test Types By Time Series - Viable Bacteria Only", y = "Viable Bacteria Log10 gc/ml (50ml)") +
  scale_fill_manual(values = c(pal[2], pal[3])) +
