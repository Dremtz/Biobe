...
airgrep <- input_cleaned[grepl('AS|Bobcat', input_cleaned$Sample.Type),]
airgroup <- ddply(airgrep, .( Sample.Day, Treatment, Time.Series), summarise, PBScounts = sum(PBScounts), PMAcounts = sum(PMAcounts))

write.csv(airgroup,"GE_sepi_airgroup.csv")

df_calc <- read.csv("sepi_df_calc.csv")
df_calc <- df_calc %>% 
  mutate(normalize = round((pma_sum/airtotal), 100000)) 
...
allair <- df_calc[grepl('Aerosol Sense|Bobcat|SKC|Universal Spot|BSV 8-liter', df_calc$Sample.Type), ]
...
viable <- input_cleaned[!grepl('0.000000', input_cleaned$PMAcounts),]
...
#viable break####
ggplot(input_cleaned, aes(x = Treatment, y = PMAcounts,)) +
  geom_boxplot(outlier.shape = NA, aes(fill = Treatment), alpha = 0.5) +
  geom_jitter(aes(fill = Treatment, color = Sample.Type,), size = 2, alpha = .7,) +
  geom_signif(comparisons = list(c("Control", "Experimental")),
              map_signif_level = FALSE, tip_length = 0.01,
