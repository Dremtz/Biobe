...
setwd ("/Users/esbl/Dropbox (University of Oregon)/Projects/Active_Air_Cleaning _Projects/DAT_Project Data/R_Data/Genesis_DATA/GE_MS2_DATA")



####READ PMA PLATE 1#####
qp1 <- read.csv('/Users/esbl/Dropbox (University of Oregon)/Projects/Active_Air_Cleaning _Projects/DAT_Project Data/R_Data/Genesis_DATA/GE_MS2_DATA/GE_RAW_MS2_PMA1.csv', header = T,skip = 23, stringsAsFactors = F)
...
qp2 <- read.csv('/Users/esbl/Dropbox (University of Oregon)/Projects/Active_Air_Cleaning _Projects/DAT_Project Data/R_Data/Genesis_DATA/GE_MS2_DATA/GE_RAW_MS2_PMA2A.csv', header = T,skip = 23, stringsAsFactors = F)
...
GE_RESULTS_MS2_PMA_COMBINED <- cbind(meta, qp1.3[match(meta$Sample.Number, qp1.3$Sample), ])
GE_RESULTS_MS2_PMA_COMBINED$Sample <- NULL




GE_RESULTS_MS2_PMA_COMBINED[is.na(GE_RESULTS_MS2_PMA_COMBINED)] <- 0

#####3 PBS PLATE 1####


qp3 <- read.csv('/Users/esbl/Dropbox (University of Oregon)/Projects/Active_Air_Cleaning _Projects/DAT_Project Data/R_Data/Genesis_DATA/GE_MS2_DATA/GE_RAW_MS2_PBS1B.csv', header = T,skip = 23, stringsAsFactors = F)
...
####4 PBS PLATE 2####
qp4 <- read.csv('/Users/esbl/Dropbox (University of Oregon)/Projects/Active_Air_Cleaning _Projects/DAT_Project Data/R_Data/Genesis_DATA/GE_MS2_DATA/GE_RAW_MS2_PBS2A.csv', header = T,skip = 23, stringsAsFactors = F)
...
standards4 <- standards4[!grepl('Standard 1', standards4$Sample),]
# Remove the standards from the data set with all the other samples    
...
meta = read.csv("/Users/esbl/Dropbox (University of Oregon)/Projects/Active_Air_Cleaning _Projects/DAT_Project Data/ONSite_GenesisAir_Metadata_R.csv")
GE_RESULTS_MS2_PBS_COMBINED <- cbind(meta, qp4.3[match(meta$Sample.Number, qp4.3$Sample), ])

GE_RESULTS_MS2_PBS_COMBINED$Sample <- NULL

#set NAs to 0
GE_RESULTS_MS2_PBS_COMBINED[is.na(GE_RESULTS_MS2_PBS_COMBINED)] <- 0
unique(GE_RESULTS_MS2_PBS_COMBINED$PBScounts)
...
MS2_bound <- inner_join(qp4.3_final, GE_RESULTS_MS2_PMA_COMBINED)
write.csv(MS2_bound, "GE_MS2_COMPLETE.csv")

MS2_COMPLETE_CLEANED <- MS2_bound[grepl('1|2|3|4|5|6B|7|8', MS2_bound$Sample.Day), ]
write.csv(MS2_COMPLETE_CLEANED, "GE_MS2_COMPLETE_CLEANED.csv")

#calculate the normilization
df_calc <- MS2_COMPLETE_CLEANED %>% 
  mutate(normalize = round((PBScounts/PMAcounts), 1000)) 
