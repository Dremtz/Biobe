...
#INPUT DATA, GROUP AND NORMALIZE#######

input <- read.csv('GE_MS2_COMPLETE_CLEANED.csv')
...
#PLOT DAILY PBS NEB TOTALS T0############ FIGURE OUT WHY COUNTS ARE SO LOW####
...
####PLOT ALL AIR SAMPLERS####
allair <- df_calc[grepl('Air Sampler A - 200l/m|Air Sampler B - 200l/m|Air Sampler D - 8 l/m|Air Sampler C - 10 l/m|Air Sampler E - 1.5 l/m', df_calc$Sample.Type), ]

ggplot(allair, aes(x = Treatment, y = normalize)) +
  geom_boxplot(outlier.shape = NA, aes(fill = Treatment), alpha = 0.5) +
  geom_jitter(aes(fill = Treatment, color = Time.Series,), size = 2, alpha = .7,) +
  geom_signif(comparisons = list(c("Control", "Experimental")),
              map_signif_level = FALSE, tip_length = 0.01,
              test = t.test) +
  theme_classic() +
  labs(title = "All Air Samplers - Virus", y = "Viable/Total Virus") +
  scale_fill_manual(values = c(pal[2], pal[3])) +
  theme(legend.title = element_blank())
...
hiflow <- df_calc[grepl('Air Sampler A - 200l/m|Air Sampler B - 200l/m', df_calc$Sample.Type), ]
...
loflow <- df_calc[grepl('Air Sampler D - 8 l/m|Air Sampler C - 10 l/m|Air Sampler E - 1.5 l/m', df_calc$Sample.Type), ]
...
##########PLOT JUST VIABLE######## 
viable <- read.csv('/Users/esbl/Dropbox (University of Oregon)/Projects/Active_Air_Cleaning _Projects/DAT_Project Data/R_Data/Genesis_DATA/MS2Viable.csv')

ggplot(viable, aes(x = Treatment, y = log10(PMAcounts),)) +
  geom_boxplot(outlier.shape = NA, aes(fill = Treatment), alpha = 0.5) +
  geom_jitter(aes(fill = Treatment, color = Sample.Type,), size = 2, alpha = .7,) +
  geom_signif(comparisons = list(c("Control", "Experimental")),
              map_signif_level = FALSE, tip_length = 0.01,
              test = t.test, y_position = 3.6) +
  theme_classic() +
  labs(title = "All Test Types By Time Series - Viable Virus Only", y = "Viable Virus Log10 gc/ml (50 ml)") +
  scale_fill_manual(values = c(pal[2], pal[3])) +
  theme(legend.title = element_blank()) +
  facet_wrap(~Time.Series)
